	...
	foreach my $date ( keys %{ $self->data_hash } ) {
		foreach my $categoria ( keys %{ $self->data_hash->{$date} } ) {
			foreach my $usuario (keys %{$self->data_hash->{$date}->{$categoria}}){
				foreach my $pagina (keys %{$self->data_hash->{$date}->{$categoria}->{$usuario}}){
					if ( exists $self->data_hash->{$categoria}->{$usuario}->{$pagina} ) {
						$self->data_hash->{$categoria}->{$usuario}->{$pagina}->{ocurrencias} +=
						  $self->data_hash->{$date}->{$categoria}->{$usuario}->{$pagina}->{ocurrencias};
					} else {
						$self->data_hash->{$categoria}->{$usuario}->{$pagina} = $self->data_hash->{$date}->{$categoria}->{$usuario}->{$pagina};
					}
				}
			}
		}
